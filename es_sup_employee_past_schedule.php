<?php
/*********************************************************
	File: es_sup_employee_past_schedules.php
...
if ($gotoedit) {
	if (!empty($s_group) && !empty($s_exptime)) {
		header("Location: es_sup_edit_employee_schedule.php?user_id=$u_id&s_group=$s_group&s_exptime=$s_exptime&".session_name()."=".session_id());
		exit;
	}
}
...
print_header($es_lang["past_schedules"]." - ".$employee["user_name"]);
print '<br><br><span class="pagetitle">'.$es_lang["past_schedules"]." - ".$employee["user_name"].'</span><br><img src="images/bar_1.gif" width="75%" height="2">';
//-- get the schedules as an array of schedules for this user
$schedules = get_past_user_schedules($employee);
...
<?php
	print "<table><tr><td>";
	print "<form name=\"scheduleform\" method=\"get\" action=\"es_sup_employee_past_schedule.php\">\n";
	print "<input type=\"hidden\" name=\"gotoedit\" value=\"0\">\n";
	print "<input type=\"hidden\" name=\"deleteschedule\" value=\"0\">\n";
	print "<input type=\"hidden\" name=\"user_id\" value=\"$u_id\">\n";
	...
	print "<input type=\"button\" value=\"".$es_lang["new"]."\" onclick=\"window.location='es_sup_edit_employee_schedule.php?user_id=$u_id&create=create&".session_name()."=".session_id()."';\">\n";
	print "</form>\n";
	print "</td></tr></table>\n";
}
...
<td>
	<table>
	<tr><td align="right"><?php print $es_lang["min_left"];?></td><td> <?php print $employee["user_min"];?></td></tr>
	<tr><td align="right"><?php print $es_lang["max_left"];?></td><td> <?php print $employee["user_max"];?></td></tr>
	<tr><td align="right"><b><?php print $es_lang["total_hours"];?></b></td><td><?php print $total_hours;?></td></tr>
	<tr><td align="right"><b><?php print $es_lang["hours_desired"];?></b></td><td><?php print $employee["user_hours"];?></td></tr>
...
<td valign="top">
	...
	<?php print $es_lang["lasst_hour"]; ?> <select name="last_hour" onchange="document.timeform.submit();">
	<?php 
		$stime = mktime($START_HOUR, 0, 0, 1,1,2003);
...
if ($view!="print") {
	print '<input type="button" name="edit" value="'.$es_lang["edit_schedule"].'" onclick="window.location = \'es_sup_edit_employee_schedule.php?user_id='.$u_id.'&s_group='.$s_group.'&s_exptime='.$s_exptime.'&'.session_name()."=".session_id().'\';">';
}
